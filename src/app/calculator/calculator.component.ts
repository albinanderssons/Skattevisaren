...
export class CalculatorComponent implements OnInit {
  ...
  ngOnInit() {
    this.service.currentData$.subscribe((data: any) => {
      ...
      const availableYears = data.map((row: any) => Number(row['år']));
      const validYears = availableYears.filter((year: number) => !isNaN(year));
      const uniqueYears = [...new Set(validYears)] as number[];
      const latestYear = Math.max(...uniqueYears);
      
      // Set up years array with latest year first
